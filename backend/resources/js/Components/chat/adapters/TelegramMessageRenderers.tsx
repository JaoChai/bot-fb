...
export function renderSticker(message: RenderableMessage): ReactNode {
...
export function renderVideo(message: RenderableMessage): ReactNode {
  if (!message.media_url) {
    return (
      <div className="bg-muted/50 rounded-lg p-4 text-center text-muted-foreground">
...
export function renderVoice(message: RenderableMessage): ReactNode {
  if (!message.media_url) {
    return (
      <p className="text-muted-foreground text-sm">
        {message.message_type === 'voice' ? 'Voice message' : 'Audio file'}
      </p>
    );
  }
...
export function renderFile(message: RenderableMessage, isUser: boolean): ReactNode {
  const metadata = message.media_metadata as Record<string, unknown> | null;
...
export function renderLocation(message: RenderableMessage, isUser: boolean): ReactNode {
  const metadata = message.media_metadata as Record<string, unknown> | null;
...
export function renderContact(message: RenderableMessage, isUser: boolean): ReactNode {
  const metadata = message.media_metadata as Record<string, unknown> | null;
...
export function renderPoll(message: RenderableMessage, isUser: boolean): ReactNode {
  const metadata = message.media_metadata as Record<string, unknown> | null;
