...
function renderImage(message: RenderableMessage, onImageClick?: (url: string) => void): ReactNode {
...
function renderVideo(message: RenderableMessage): ReactNode {
  if (!message.media_url) {
    return (
      <div className="bg-muted/50 rounded-lg p-4 text-center text-muted-foreground">
  ...
  return (
    <div className="max-w-[280px]">
      <video src={message.media_url} controls className="rounded-lg w-full" preload="metadata">
...
function renderAudio(message: RenderableMessage): ReactNode {
  if (!message.media_url) {
...
function renderFile(message: RenderableMessage, isUser: boolean): ReactNode {
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        {metadata?.file_size != null && (
          <p className="text-xs text-muted-foreground">{formatFileSize(metadata.file_size as number)}</p>
        )}
      </div>
...
function renderSticker(message: RenderableMessage): ReactNode {
  if (message.media_url) {
    return <img src={message.media_url} alt="Sticker" className="max-w-[120px] max-h-[120px]" loading="lazy" />;
  }
...
export const facebookAdapter: ChannelAdapter = {
  ...
  renderMessageContent: (message: RenderableMessage, options) => {
    ...
    switch (type) {
      ...
      case 'text':
      default:
        return <p className="whitespace-pre-wrap break-words">{message.content || '[Unsupported message type]'}</p>;
    }
  },
  ...
  renderAvatar: (customer: Customer | null, size = 'md') => {
    ...
    return (
      <Avatar className={cn(sizeClass, 'flex-shrink-0')}>
        {customer?.picture_url ? <AvatarImage src={customer.picture_url} alt={customer.display_name || 'User'} /> : null}
  ...
  supportsFeature: (feature) => ['sticker', 'quickReply'].includes(feature),
};
