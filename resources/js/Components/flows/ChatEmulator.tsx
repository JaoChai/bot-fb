/**
 * ChatEmulator - Chat emulator for testing flow responses
 *
 * Copied from frontend and adapted for Inertia context
 * TODO: Implement streaming chat with Inertia patterns
 */
...
// Placeholder types for streaming chat (to be implemented with Inertia)
  ...
  return (
    <div className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
      <div className={`max-w-[85%] ${message.role === 'assistant' ? 'w-full' : ''}`}>
        {/* TODO: Add ProcessDisplay component for assistant messages */}
        <div
    ...
    <div className={cn("w-96 border-l bg-card flex flex-col", className)}>
      ...
      <div className="flex items-center justify-between px-4 py-3 text-background bg-foreground border-b">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold">Chat Emulator</span>
        </div>
        <div className="flex items-center gap-2">
          <Button
            size="sm"
        ...
        {messages.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full text-center">
            ...
            <p className="text-xs text-muted-foreground/70 mt-1">
              {disabled ? disabledReason : 'Type a message below to start'}
            </p>
          </div>
        ) : (
          messages.map((msg) => (
      ...
      <div className="border-t p-4 space-y-3 bg-background/50">
        <div className="flex gap-2">
          <input
            type="text"
            placeholder={isStreaming ? 'Processing...' : disabled ? disabledReason : 'Type a message...'}
