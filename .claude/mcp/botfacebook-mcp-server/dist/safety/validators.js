 ...
 */
export function validateTinkerCode(code) {
    ...
    for (const pattern of dangerousPatterns) {
        if (pattern.test(code)) {
            throw new SafetyError("Dangerous code pattern detected. This operation is not allowed.", "DANGEROUS_CODE");
        }
    }
}
/**
 * Validate bot ID
 */
export function validateBotId(botId, required = false) {
 ...
 */
export function validateTestCount(count) {
    if (count === undefined)
        return 20; // default
 ...
 */
export function validateMessage(message, required = false) {
 ...
 */
export function validateRailwayService(service) {
    if (!service)
        return "backend";
    if (!["backend", "frontend", "reverb"].includes(service)) {
        throw new SafetyError("service must be one of: backend, frontend, reverb", "VALIDATION_ERROR");
    }
    return service;
}
