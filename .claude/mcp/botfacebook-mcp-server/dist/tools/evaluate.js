...
export async function handleEvaluate(input, config) {
    const { action, bot_id, evaluation_id, test_case_id, config: evalConfig, evaluation_ids, page, per_page, status, } = input;
    try {
        ...
        switch (action) {
            ...
            case "list": {
                ...
                const response = await client.listEvaluations(botId, pagination.page, status);
            ...
            case "test_case_detail": {
                ...
                const response = await client.get(`/api/bots/${botId}/evaluations/${evaluation_id}/test-cases/${test_case_id}`);
            ...
            case "compare": {
                ...
                const evaluations = await Promise.all(evaluation_ids.map((id) => client.getEvaluation(botId, id)));
                // Build comparison
                const comparison = evaluations.map((eval_result, index) => {
                    const data = eval_result.data || {};
                    return {
