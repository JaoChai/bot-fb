...
export async function handleBotManage(
  input: BotManageInput,
  config: ServerConfig
): Promise<ToolResult> {
  const {
    action,
  ...
  try {
    ...
    if (action.startsWith("delete_")) {
      validateConfirmation("bot_manage", action, input.data?.confirm as boolean);
    }
    ...
    switch (action) {
      ...
      case "duplicate_flow": {
        ...
        const response = await client.post(
          `/api/bots/${bId}/flows/${flow_id}/duplicate`
        );
      ...
      case "set_default_flow": {
        ...
        const response = await client.post(
          `/api/bots/${bId}/flows/${flow_id}/set-default`
        );
      ...
      case "upload_document": {
        return createErrorResult(
          "Document upload requires file handling. Use the web interface or API directly."
        );
      }
