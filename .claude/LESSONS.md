...
## 🚨 บทเรียน 3 ม.ค. 2026 - เสียเวลาทั้งวันเพราะเดา

### 11. ดูข้อมูลจริงก่อน อย่าเดา!

**ปัญหา Toggle ไม่ทำงาน:**

| ❌ สิ่งที่ผมทำ (เสียเวลา 10+ commits) | ✅ สิ่งที่ควรทำ (10 วินาที) |
|-------------------------------------|---------------------------|
| เดาว่า TanStack Query cache มีปัญหา | เปิด BotController.php ดู return อะไร |
| เดาว่า optimistic update มีปัญหา | เปิด useConnections.ts ดู expect อะไร |
| เดาว่า localStorage persist มีปัญหา | เทียบ 2 อัน แก้ให้ตรงกัน |
| แก้ไปแก้มา based on assumptions | **จบใน 1 commit** |

**Root cause จริง:**
```
Backend return: { message, data: Bot, webhook_setup }
Frontend expect: Bot

แก้: return response.data แทน response
```

**Issue #86 เขียนไว้ชัดเจน แต่ผมไม่อ่าน/ไม่ทำตาม**

### 12. ปัญหา Cache - ก็เดาอีก

| ❌ สิ่งที่ผมทำ | ✅ สิ่งที่ควรทำ |
|--------------|---------------|
| อธิบาย buster ยาวเหยียด (ไม่เกี่ยว) | `curl -I production` ดู headers จริง |
| อธิบาย TanStack Query cache (ไม่เกี่ยว) | เห็นว่าไม่มี Cache-Control |
| ไม่ได้ตรวจ production จริง | แก้ server ให้ส่ง headers → จบ |

### 13. กฎเหล็ก - ดูก่อนเดา

```
┌─────────────────────────────────────────────────────────────┐
│  🛑 ก่อนแก้ Bug ทุกครั้ง                                      │
├─────────────────────────────────────────────────────────────┤
│  1. มี GitHub Issue ไหม? → อ่านก่อน อาจมีคำตอบแล้ว          │
│  2. Backend return อะไร? → เปิดไฟล์ดู / curl ดู              │
│  3. Frontend expect อะไร? → เปิดไฟล์ดู                       │
│  4. Production ทำงานยังไง? → curl -I / inspect จริง          │
│  5. ข้อมูลตรงกันไหม? → ถ้าไม่ตรง แก้ให้ตรง → จบ             │
└─────────────────────────────────────────────────────────────┘
...
├─────────────────────────────────────────────────────────────┤
│  • "น่าจะเป็น..." → ลุยแก้เลย                                │
│  • "apiPut น่าจะ extract แล้ว..." → ไม่ดูโค้ด                │
│  • "backend น่าจะ return ตรงๆ..." → ไม่เปิด Controller        │
│  • คิดซับซ้อน ทั้งที่ปัญหาง่าย                                │
└─────────────────────────────────────────────────────────────┘
```

### 14. Checklist ก่อนแก้ API/Data Issues

```
□ เปิด Backend Controller ดู return structure
□ เปิด Frontend hook/component ดู expected type
□ ทั้งสองตรงกันไหม?
□ ถ้าไม่ตรง → แก้ฝั่งใดฝั่งหนึ่งให้ตรง
□ Test แล้ว work → commit
```

### 15. Checklist ก่อนแก้ Cache Issues

```
□ curl -I production ดู actual headers
□ Headers ถูกต้องไหม?
□ ถ้าไม่ถูก → แก้ server config
□ Test local ก่อน deploy
□ Deploy แล้ว curl -I อีกครั้ง verify
```

---

*อัพเดทล่าสุด: 3 ม.ค. 2026 - บทเรียนจากความผิดพลาดวันนี้*
