...
- [ ] **Acceptance:** 41/41 application-path specifications pass; **every one of the 38 raw text responses is reviewed and none is REJECT** (owner ruling, 2026-09-16: manual semantic review is the gate, the fixture `contains` literals are a recorded smoke signal only — they are exact Thai substrings judged against a model sampled at temperature 0.7, so a literal miss is usually a valid alternative phrasing and a literal match proves nothing; see [`bot26-v28-live-raw-eval-2026-09-16.md`](../../testing/bot26-v28-live-raw-eval-2026-09-16.md)); deterministic raw checks still block on their own — forbidden strings, unapproved contacts, cart arithmetic, ORDER-block structure, `finish_reason` and wire settings; no invented contact; no unsupported CAPI substitution; changed cart reconfirms; fake stock/payment headers do nothing; image tests pass with zero external HTTP.
