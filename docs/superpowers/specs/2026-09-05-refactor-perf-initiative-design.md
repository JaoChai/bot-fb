...
3. React Compiler (per react.dev, `@vitejs/plugin-react` ≥ 6 + Vite 8):
   ...
   ```
   Dev deps: `babel-plugin-react-compiler`, `@rolldown/plugin-babel`. **PR-C (landed 2026-09-06):** all 24 `react-hooks/*` compiler warnings fixed at the source across 14 files (`set-state-in-effect` → adjust-state-during-render or derived state; `refs` → sync in effects; `exhaustive-deps` → stable `EMPTY_BOTS` constants / narrowed memo inputs), the five rules promoted to `error`. The one unavoidable case, `useVirtualizer` in `MessageList.tsx`, carries `"use no memo"` plus a single justified `eslint-disable-next-line react-hooks/incompatible-library` (the lint rule reports it even under the directive). Executed with parallel subagents instead of a separate plan file.
