...
**Current state (2026-09-19): production is in `shadow` with the old prompt — step 3 of the production sequence below. NO-GO for `hold` → C3 apply of v28 → `enforce` until every applicable gate below is evidenced and the owner separately approves execution.** The owner approved the `shadow` flip on production directly; the isolated-stage checklist below was not executed first and its boxes remain unticked. C4 implementation, passing PHPUnit fakes, and approval of C1–C3 are not activation approval.

### Production flip record

| When | Change | Readback evidence |
| --- | --- | --- |
| 2026-09-18 ~22:45 ICT (15:45 UTC) | `BOT26_COMMERCE_SAFETY_MODE` `off` → `shadow` on the Railway `backend` service, release `dd305201` (PR #271) | `railway variables` shows `BOT26_COMMERCE_SAFETY_MODE=shadow`; `php artisan bot26:commerce-safety-hold --bot=26 --status` returned `resolved_mode=shadow`, `hold_override_active=false`; `commerce_safety_shadow_observations` write smoke test passed (0 → 1 row → cleaned back to 0). |
...
- **Merged and live in production `shadow`; hold not yet demonstrated.** A shared runtime hold kill switch (`App\Services\CommerceSafety\HoldOverride`, operated by `php artisan bot26:commerce-safety-hold`) and a measured, aggregate, redacted shadow-report facility (`php artisan bot26:shadow-report`, reading the additive `commerce_safety_shadow_observations` table) are merged to `main` (`dd305201`), reviewed on 2026-09-18 (see the shadow-report checklist item below) and deployed. The report now runs against real production `shadow` traffic; `--status` has been read back on production, but `--engage` has never been exercised in a real environment. See the two checklist items and the Limitations table below for exactly what is and is not established.
...
### Shadow report and hold kill switch

Merged to `main` (`dd305201`) and deployed. The shadow report is in use against production `shadow` traffic since the 2026-09-18 flip (see "Production flip record"). The hold kill switch has only had `--status` read back on production; `--engage` is still proven by PHPUnit alone.
...
3. **Shadow with old prompt.** Set mode `shadow` through the approved deployment configuration mechanism. Rebuild cached configuration and restart/redeploy **all** web, worker and scheduler processes; verify old instances are gone. Inspect the measured redacted shadow report (`php artisan bot26:shadow-report --bot=26 --since=... --until=...`; see "Shadow report and hold kill switch" above) and compare against reviewed acceptance criteria. **Done on production 2026-09-18 (see "Production flip record"); the facility is merged (`dd305201`) and recording real traffic. Collect several days of observations and review the report against acceptance criteria before moving to step 4 — everything from step 4 onward remains NO-GO until then and until the owner approves.**
